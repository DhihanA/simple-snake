...
class Snake:
    ...
    def move(self):
        ...
        if not self.check_bounds():
            ...
            # print("we are in the out of bounds now!")
            if self.direction == Direction.DOWN:
                next_head = (curr_head[0], 0)
                self.body.append(next_head)
            elif self.direction == Direction.UP:
                next_head = (curr_head[0], self.bounds[1] - self.block_size)
                self.body.append(next_head)
            elif self.direction == Direction.RIGHT:
                next_head = (0, curr_head[1])
                self.body.append(next_head)
            elif self.direction == Direction.LEFT:
                next_head = (self.bounds[0] - self.block_size, curr_head[1])
                self.body.append(next_head)
    ...
    def check_bounds(self):
        ...
        if (head[0] < 0) or (head[0] >= self.bounds[0]):
            return True
        if (head[1] < 0) or (head[1] >= self.bounds[1]):
            return True
